#!/bin/bash
#SBATCH --job-name=bwformer-tallinn-resume
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=shard:1
...
RESUME_CKPT="${RESUME_CKPT:-${PROJECT_DIR}/checkpoints/checkpoint_best.pth}"
EXP_NAME="${EXP_NAME:-tallinn_resume_from_best}"
...
RESUME_EPOCH="$(
  CHECKPOINT_PATH="${RESUME_CKPT}" python - <<'PY'
import argparse
import os
import torch
...
if [ "${MAX_CORNER_NUM}" != "${CKPT_MAX_CORNER_NUM}" ]; then
  ...
  echo "This checkpoint expects 2 * max_corner_num corner queries inside corner_models_3d." >&2
  echo "Please keep MAX_CORNER_NUM=${CKPT_MAX_CORNER_NUM} when resuming from this checkpoint." >&2
  exit 1
fi

TOTAL_EPOCHS=$((RESUME_EPOCH + EXTRA_EPOCHS + 1))

echo ""
echo "[2/2] Resume BWFormer training"
echo "Resume epoch: ${RESUME_EPOCH}"
echo "Extra epochs: ${EXTRA_EPOCHS}"
echo "Target total epochs (--epochs): ${TOTAL_EPOCHS}"
