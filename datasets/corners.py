...
class CornersDataset(Dataset):
    ...
    def process_data(self, data):
        ...
        if not self.inference:
            ...
            roof_line_labels = self.get_roof_line_labels(annot)
            all_data['pixel_labels'] = pixel_labels
            all_data['gauss_labels'] = gauss_labels
            all_data['roof_line_labels'] = roof_line_labels
    ...
    def get_roof_line_labels(self, annot, line_width=1, sigma=1.5):
        labels = np.zeros((self.image_size, self.image_size), dtype=np.float32)
        drawn_edges = set()

        for corner, connections in annot.items():
            start = np.array(corner, dtype=np.float32)[:2]
            for other_corner in connections:
                end = np.array(other_corner, dtype=np.float32)[:2]
                start_key = tuple(np.round(start).astype(np.int32).tolist())
                end_key = tuple(np.round(end).astype(np.int32).tolist())
                edge_key = tuple(sorted((start_key, end_key)))
                if edge_key in drawn_edges:
                    continue
                drawn_edges.add(edge_key)

                x1 = int(np.clip(round(start[0]), 0, self.image_size - 1))
                y1 = int(np.clip(round(start[1]), 0, self.image_size - 1))
                x2 = int(np.clip(round(end[0]), 0, self.image_size - 1))
                y2 = int(np.clip(round(end[1]), 0, self.image_size - 1))
                cv2.line(labels, (x1, y1), (x2, y2), 1.0, line_width)

        if labels.max() <= 0:
            return labels

        labels = gaussian_filter(labels, sigma=sigma)
        labels = labels / max(labels.max(), 1e-6)
        return labels.astype(np.float32)

    def resize_data(self, image, annot, det_corners=None):
