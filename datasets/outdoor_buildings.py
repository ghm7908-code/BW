...
class OutdoorBuildingDataset(CornersDataset):
    def __init__(self, data_path, det_path, phase='train', image_size=256, rand_aug=True,
                 inference=False, list_path=None, corner_limit=None):
        ...
        self.corner_limit = corner_limit
        ...
        if phase == 'train':
            default_list = os.path.join(data_path, 'train_list.txt')
            self.training = True
        elif phase == 'valid':
            default_list = os.path.join(data_path, 'valid_list.txt')
            self.training = False
        elif phase == 'test':
            default_list = os.path.join(data_path, 'test_list.txt')
            self.training = False
        else:
            raise ValueError('Invalid phase {}'.format(phase))

        datalistfile = list_path or default_list
        if not os.path.exists(datalistfile):
            if phase in ['valid', 'test']:
                datalistfile = os.path.join(data_path, 'train_list.txt')
            else:
                raise FileNotFoundError('Cannot find split file {}'.format(datalistfile))

        with open(datalistfile, 'r') as f:
            self._data_names = [line.strip() for line in f if line.strip()]
    ...
    def __getitem__(self, idx):
        ...
        if self.corner_limit is not None and not self.inference and len(corners) > self.corner_limit:
